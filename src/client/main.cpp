...
int main(int argc, char **argv)
{
    ...
    sockaddr_in server;
    memset(&server, 0, sizeof(sockaddr_in)); // 初始化 sockaddr_in 类型的结构体
    ...
    for (;;)
    {
        ...
        case 1: // login
        {
            ...
            sem_wait(&rwsem); //等待信号量，子线程处理完登录的响应消息后通知这里
...
void readTaskHandler(int clientfd)
{
    for(;;)
    {
        ...
        if(-1 == len || 0 == len) // 返回0表示连接被正常关闭,返回-1表示发生错误
        {
            close(clientfd);
            exit(-1);
        }
        ...
        {
            doLoginResponse(js); // 处理登陆响应的业务
            sem_post(&rwsem); // 通知主线程登录响应处理完成
            continue;
        }
